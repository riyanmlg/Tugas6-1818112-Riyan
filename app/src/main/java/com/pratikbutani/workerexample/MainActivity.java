			...
			public void onClick(View v) {
				if (mainBinding.appCompatButtonStart.getText().toString().equalsIgnoreCase(getString(R.string.button_text_start))) {
					...
					PeriodicWorkRequest periodicWork = new PeriodicWorkRequest.Builder(MyWorker.class, 15, TimeUnit.MINUTES)
							.addTag(TAG)
							.build();
					WorkManager.getInstance().enqueueUniquePeriodicWork("Lokasi", ExistingPeriodicWorkPolicy.REPLACE, periodicWork);

					Toast.makeText(MainActivity.this, "Work manager lokasi dimulai : " + periodicWork.getId(), Toast.LENGTH_SHORT).show();
		...
		if (requestCode == PERMISSION_REQUEST_CODE) {
			if (grantResults.length > 0) {
				...
				if (coarseLocation && fineLocation)
					Toast.makeText(this, "Perizinan diterima", Toast.LENGTH_SHORT).show();
				else {
					Toast.makeText(this, "Perizinan ditolak", Toast.LENGTH_SHORT).show();
				}
			}
		}
	}
